...
! *********************************************************************** 
      ...
      integer function extras_binary_check_model(binary_id)
         ...
         integer, intent(in) :: binary_id
         integer :: ierr
         real(dp) :: period_target, period_target_thresh
         ...
         period_target = 2110.0*86400. !!b%s1%xtra(1) * 86400
         period_target_thresh = 1*86400. !!b%s1%xtra(2) * 86400

         
         write(*,*) "period [d] = ",b%period/86400., "   target, thresh =", period_target/86400., period_target_thresh/86400.
         if(b%period < period_target) then
            write(*,*) "period less than target"
            if( abs(b%period - period_target) < period_target_thresh) then
